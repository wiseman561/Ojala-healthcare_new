...
var jwtIssuer = builder.Configuration["JwtSettings:Issuer"] ?? "Ojala.Identity";
var jwtAudience = builder.Configuration["JwtSettings:Audience"] ?? "Ojala.Client";
...
builder.Services
  .AddHealthChecks()
  .AddUrlGroup(new Uri("http://ojala-api:80/health"), name: "api");

// 5) Add Swagger configuration
builder.Services.AddSwaggerGen(c =>
{
  c.SwaggerDoc("v1", new OpenApiInfo { Title = "Ojala API Gateway", Version = "v1" });
...
app.UseSwagger();
app.UseSwaggerUI(c => c.SwaggerEndpoint("/swagger/v1/swagger.json", "Ojala API Gateway v1"));
...
app.MapGet("/", () => "Ojala API Gateway - Routes: /api/patients, /api/appointments, /api/auth, /api/health");
